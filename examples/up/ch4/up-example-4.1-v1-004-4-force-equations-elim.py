...
display_equations_(tmp, values, want=R.x)
# F1_y = -F2_y - F3_y + R_y
# F1_mag*cos(F1_angle) = R_x - F2_mag*cos(F2_angle) - F3_mag*cos(F3_angle)
# F1_y = F1_mag*sin(F1_angle)
# F1_mag = sqrt(F1_y**2 + (-R_x + F2_mag*cos(F2_angle) + F3_mag*cos(F3_angle))**2)
# F1_angle = atan2(F1_y, R_x - F2_mag*cos(F2_angle) - F3_mag*cos(F3_angle))
# F2_y = F2_mag*sin(F2_angle)
# F2_mag = sqrt(F2_mag**2*cos(F2_angle)**2 + F2_y**2)
# F2_angle = atan2(F2_y, F2_mag*cos(F2_angle))
# F3_y = F3_mag*sin(F3_angle)
# F3_mag = sqrt(F3_mag**2*cos(F3_angle)**2 + F3_y**2)
# F3_angle = atan2(F3_y, F3_mag*cos(F3_angle))
# R_x = R_mag*cos(R_angle)
# R_y = R_mag*sin(R_angle)
# R_mag = sqrt(R_x**2 + R_y**2)
# R_angle = atan2(R_y, R_x)
...
# Solving for unknowns: [F1_y, R_y, R_angle_deg, R_angle, F3_y, R_x, R_mag, F2_y]
# Solution 1:
...
# We end up with 5 solutions!
#
# `tmp` has the following equation:
#
# F1_mag*cos(F1_angle) = R_x - F2_mag*cos(F2_angle) - F3_mag*cos(F3_angle)
#
# which is very easy to solve for R_x
#
# Consider:
# Enhance the solver to use an equation like that if it's in the set.
...
display_equations_(tmp, values, want=R.y)
# F1_mag*cos(F1_angle) = -F2_mag*cos(F2_angle) - F3_mag*cos(F3_angle) + R_x
# F1_mag*sin(F1_angle) = 0
# F1_mag = sqrt((F2_mag*cos(F2_angle) + F3_mag*cos(F3_angle) - R_x)**2)
# F1_angle = atan2(0, -F2_mag*cos(F2_angle) - F3_mag*cos(F3_angle) + R_x)
# F2_mag*sin(F2_angle) = 0
# F2_mag = sqrt(F2_mag**2*cos(F2_angle)**2)
# F2_angle = atan2(0, F2_mag*cos(F2_angle))
# R_y = F3_mag*sin(F3_angle)
# F3_mag = sqrt(R_y**2 + F3_mag**2*cos(F3_angle)**2)
# F3_angle = atan2(R_y, F3_mag*cos(F3_angle))
# R_x = R_mag*cos(R_angle)
# R_y = R_mag*sin(R_angle)
# R_mag = sqrt(R_y**2 + R_x**2)
# R_angle = atan2(R_y, R_x)
# R_angle_deg = 180*R_angle/pi

solve_and_display_(tmp, values, R.y) # takes a very long time

# This equation is in the set:
# 
# R_y = F3_mag*sin(F3_angle)
#
# The left side is the symbol we want to solve for.
# The right side has symbols that we know the values of.
#
# So it seems like the solver should be able to handle this one directly.
